...
extension SessionManager {
    func handleReceivedMessage(_ motionData: MotionData) {
           let formattedDate = csvDateFormatter.string(from: Date(timeIntervalSince1970: motionData.timestamp))
    ...
    func exportDataToCSV(completion: @escaping (URL) -> Void) {
           let currentDateTimeString = fileNameDateFormatter.string(from: Date())
